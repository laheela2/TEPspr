...
<script src="/TEP/static/js/openmeetmodify.js"></script>

<script>
	window.onload = function() {
		CKEDITOR.replace('o_content',{
				'filebrowserUploadUrl':'ckeditorImageUploadModify.action?o_no=<s:property value="o_no"/>'
		});
		
		var f = document.omf_form;
		f.o_subject.value='<s:property value="o_subject"/>';
		f.o_msdate.value='<s:property value="o_msdate"/>';
		f.o_medate.value='<s:property value="o_medate"/>';
		f.o_rsdate.value='<s:property value="o_rsdate"/>';
		f.o_redate.value='<s:property value="o_redate"/>';
		f.o_addr.value='<s:property value="o_addr"/>';
		f.o_title.value='<s:property value="o_title"/>';
		f.o_total_pnum.value='<s:property value="o_total_pnum"/>';
		f.o_payment.value='<s:property value="o_payment"/>';
	}
</script>
...
<s:form name="omf_form" action="openmeetModifyPro" method="post" enctype="multipart/form-data" theme="simple">
...
<table cellpadding=0 cellspacing=0>
<tr>
<td><input id="search_keyword_addr" type="text" name="" size="53%" placeholder="장소명, 주소를 검색해 주세요." onkeydown="searchAddrEnterKey();" maxlength="25"></td>
...
<textarea id="o_content" name="o_content"><s:property value="o_content"/></textarea>
</td>
</tr>
